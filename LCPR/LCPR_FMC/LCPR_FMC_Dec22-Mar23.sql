...
----------------------------- LCPR FMC TABLE - V2 ---------------------------------------
-----------------------------------------------------------------------------------------
--------------- Comment (20/7/2023): Use this code for March 2023 and before ---------------

--CREATE TABLE IF NOT EXISTS "db_stage_dev"."lcpr_fmc_table_feb_mar17" AS
...
,fixed_table AS (
SELECT *
FROM "db_stage_dev"."lcpr_fixed_table_jan_apr14"
WHERE fix_s_dim_month = (SELECT input_month FROM parameters)
)

,mobile_table AS (
SELECT *
FROM "db_stage_dev"."lcpr_postpaid_table_jan_apr14"
