...
SELECT
    *, 
    first_value(date(subscription_activation_date)) over (partition by subscription_id order by dt asc) as activation_dt, 
    1 as row_nm
...
SELECT
    *, 
    first_value(date(subscription_activation_date)) over (partition by subscription_id order by dt asc) as activation_dt,
    row_number() over (partition by subscription_id order by dt desc) as row_nm, 
...
WHERE 
    ...
    and subscription_id in (SELECT fake_churn_id FROM fake_churns)
)
...
WHERE
    subscription_id not in (SELECT fake_churn_id FROM fake_churns)
)

,BOM_active_base AS (
SELECT  subscription_id AS account
        ,customer_id AS parent_account
        ,(date(dt_2) + interval '1' month - interval '1' day) AS mob_b_dim_date
        ,CASE 
            WHEN subscription_id not in (SELECT subscription_id FROM missing_accounts) THEN DATE_DIFF('day',date(activation_dt),(date(dt_2) + interval '1' month - interval '1' day)) 
            WHEN subscription_id in (SELECT subscription_id FROM missing_accounts) THEN DATE_DIFF('day',date(activation_dt),(date(dt_2) + interval '1' month - interval '1' day - interval '1' month))
            END AS mob_b_mes_TenureDays
        ,date(activation_dt) AS mob_b_att_MaxStart
        ,CASE   
        --- Early tenure
        WHEN subscription_id not in (SELECT subscription_id FROM missing_accounts) and DATE_DIFF('day',date(activation_dt),(date(dt_2) + interval '1' month - interval '1' day)) <= 180 THEN 'Early-Tenure'
        WHEN subscription_id in (SELECT subscription_id FROM missing_accounts) and DATE_DIFF('day',date(activation_dt),(date(dt_2) + interval '1' month - interval '1' day - interval '1' month)) <= 180 THEN 'Early-Tenure'
        --- Mid tenure
        WHEN subscription_id not in (SELECT subscription_id FROM missing_accounts) and DATE_DIFF('day',date(activation_dt),(date(dt_2) + interval '1' month - interval '1' day)) <= 360 THEN 'Mid-Tenure'
        WHEN subscription_id in (SELECT subscription_id FROM missing_accounts) and DATE_DIFF('day',date(activation_dt),(date(dt_2) + interval '1' month - interval '1' day - interval '1' month)) <= 360 THEN 'Mid-Tenure'
        --- Late tenure
        WHEN subscription_id not in (SELECT subscription_id FROM missing_accounts) and DATE_DIFF('day',date(activation_dt),(date(dt_2) + interval '1' month - interval '1' day)) >  360 THEN 'Late-Tenure'
        WHEN subscription_id in (SELECT subscription_id FROM missing_accounts) and DATE_DIFF('day',date(activation_dt),(date(dt_2) + interval '1' month - interval '1' day - interval '1' month)) >  360 THEN 'Late-Tenure'
...
SELECT  subscription_id as account
        ...
        ,DATE_DIFF('day',date(activation_dt),(date(dt_2) + interval '1' month - interval '1' day)) AS mob_e_mes_TenureDays
...
SELECT  (SELECT input_month FROM parameters) AS mob_s_dim_month
        ...
        ,case 
            when B.account in (SELECT subscription_id FROM missing_accounts) then 1
...
FROM "lcpr.stage.dev"."test_lcpr_mob_postpaid_mrc"
...
ON cast(A.mob_s_att_ParentAccount as varchar) = cast(B.parent as varchar)
...
ON cast(A.mob_s_att_ParentAccount as varchar) = cast(C.parent as varchar)
GROUP BY 1,2,3
)
...
SELECT  subscription_id AS rejoiner_account
FROM cust_mstr_adj
WHERE date(dt) = (SELECT input_month FROM parameters) - interval '2' month
    ...
    AND region_name <> 'VI'
    AND subscription_status = 'A'
    AND subscription_id NOT IN (SELECT DISTINCT subsrptn_id
                            FROM "lcpr.stage.dev"."tbl_pstpd_cust_cxl_incr_data"
                            WHERE DATE(dt) = (SELECT input_month FROM parameters) - interval '1' month
                            AND cust_sts = 'O'
                            AND acct_type_cd = 'I'
