...
SELECT  subsrptn_id AS account
        ...
        ,DATE_DIFF('day',date(subsrptn_actvtn_dt),(date(dt_fix) + interval '1' month - interval '1' day)) AS mob_b_mes_TenureDays
        ,date(subsrptn_actvtn_dt) AS mob_b_att_MaxStart
        ,CASE   WHEN DATE_DIFF('day',date(subsrptn_actvtn_dt),(date(dt_fix) + interval '1' month - interval '1' day)) <= 180 THEN 'Early-Tenure'
                WHEN DATE_DIFF('day',date(subsrptn_actvtn_dt),(date(dt_fix) + interval '1' month - interval '1' day)) <= 360 THEN 'Mid-Tenure'        
                WHEN DATE_DIFF('day',date(subsrptn_actvtn_dt),(date(dt_fix) + interval '1' month - interval '1' day)) >  360 THEN 'Late-Tenure'
                    ELSE NULL END AS mob_b_fla_Tenure
        ,null AS mob_b_mes_MRC
        ,1 AS mob_b_mes_numRGUS
FROM cust_mstr_ss_fix
...
SELECT  subsrptn_id as account
        ...
        ,DATE_DIFF('day',date(subsrptn_actvtn_dt),(date(dt_fix) + interval '1' month - interval '1' day)) AS mob_e_mes_TenureDays
