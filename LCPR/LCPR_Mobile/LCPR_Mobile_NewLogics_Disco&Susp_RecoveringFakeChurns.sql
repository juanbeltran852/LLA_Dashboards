...
WHERE 
    ...
    AND lower(acct_sts_rsn_desc) not like '%contract%accept%'
...
SELECT  *
        ,CASE   WHEN (mob_e_mes_numRGUS - mob_b_mes_numRGUS) = 0 THEN '1.SameRGUs' 
                ...
                WHEN (mob_b_mes_numRGUS IS NULL AND mob_e_mes_numRGUS > 0 AND newcust_candidate_flag is not null AND A.mob_s_att_account not in (SELECT fake_churn_id FROM fake_churns)) THEN '4.New Customer'
                WHEN (mob_b_mes_numRGUS IS NULL AND mob_e_mes_numRGUS > 0 AND newcust_candidate_flag is null AND A.mob_s_att_account not in (SELECT fake_churn_id FROM fake_churns)) THEN '5.Come Back to Life'
...
--     B.lst_susp_rsn_desc
-- FROM full_flags A
-- LEFT JOIN "lcpr.stage.dev"."tbl_pstpd_cust_cxl_incr_data" B
--     ON A.mob_s_att_account = B.subsrptn_id and date(A.mob_s_dim_month) = date_trunc('month', date(B.dt)) 
-- WHERE 
--     A.mob_b_att_active + A.mob_e_att_active >= 1
--     and cast(A.mob_s_att_account as varchar) in ('7872200680', '7872355566', '7873084424', '7873086985', '7873601540', '7874003884', '7874367378', '7874772267', '7875857903', '7875901235', '7875907746', '7875907949', '7876003125', '7876004427', '7876004434', '7876640441', '7876648013', '7877182218', '7879081125', '7879082706', '7879087088')
--     and DATE(B.dt) = (SELECT input_month FROM parameters)
--         --AND cust_sts = 'O'
--         AND B.acct_type_cd = 'I'
--         AND B.rgn_nm <> 'VI'
        --AND subsrptn_sts <> 'A'
