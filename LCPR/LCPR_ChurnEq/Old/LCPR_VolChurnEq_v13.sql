...
SELECT
    ...
    --- Dashboard flags
    case
        when dx_not_retained is not null then 'Voluntary'
        when not_dx_not_retained is not null then 'Incomplete'
    end as dx_type, 
    
    --- Vol Churn Eq flags
    cc_attempts as retention_isle_intents,
    cc_attempts, 
    not_dx_not_retained, 
    -- dx_no_cc, 
    case 
        when fix_s_fla_mainmovement = '3.Downsell' or (fix_s_fla_mainmovement = '6.Null last day' and fix_s_fla_churntype = '1. Fixed Voluntary Churner') then 1
    else 0 end as intention_id_count,
    
    --- Accounts
    count(distinct dna_id*retained) as retained,
    
    --- RGUs
    sum(num_rgus_diss*dx_not_retained) as num_rgus_diss,
...
--     sum(intention_id_count*rgus_bom) as total_churn_intents, 
    
--     cast((sum(intention_id_count*rgus_bom) - sum(if(not_dx_not_retained = 1, rgus_bom*intention_id_count)) - 9400) as double)/cast(sum(intention_id_count*rgus_bom) as double) as ret_rate,
...
--     sum(if(not_dx_not_retained = 1, rgus_bom*intention_id_count)) as non_completed_churn_orders_cc
