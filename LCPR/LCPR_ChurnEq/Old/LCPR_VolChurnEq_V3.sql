...
SELECT
    ...
    case when C.ret_candidate is not null and D.completeddx_cust_id is null then C.attempt_cust_id else null end as ret_cust_flag, 
    case when C.attempt_cust_id is not null and C.ret_candidate is null then C.attempt_cust_id else null end as notret_cust_flag,
    case when C.attempt_cust_id is not null and C.ret_candidate is null and D.completeddx_cust_id is null then C.attempt_cust_id else null end as not_compdx_notret_flag
...
SELECT 
    ...
    count(distinct not_compdx_notret_flag) as not_compdx_notret
