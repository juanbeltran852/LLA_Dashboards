...
SELECT
    ...
    -- case when (dx_no_cc is not null /*or bajas_no_cursadas is not null*/) then 1 else 0 end as dx_no_cc,
    case 
        ...
        -- when fix_s_fla_mainmovement = '3.Downsell' and (bajas_no_cursadas is not null) then 1
    ...
    count(distinct case when dx_no_cc is not null or ret_account_id is not null then dna_id else null end) as all_attempts_cust,
    count(distinct case when ret_account_id is not null then dna_id else null end) as cc_attempts_cust, 
    count(distinct case when retained_flag is not null then dna_id else null end) as retained_cust,
    count(distinct case when ret_account_id is not null and retained_flag is null and vol_dx_flag is not null then dna_id else null end) as dx_not_ret_cust,
    count(distinct case when ret_account_id is not null and retained_flag is null and vol_dx_flag is null then dna_id else null end) as not_dx_not_retained_cust,
    count(distinct case when dx_no_cc is not null then dna_id else null end) as dx_no_cc_cust
