...
, dna_rgus_pre as (
SELECT
    cast(sub_acct_no_sbb as varchar) as dna_id, 
...
SELECT
    ...
    sum(bb_bom) as bb_bom,
    sum(tv_bom) as tv_bom,
    sum(vo_bom) as vo_bom,
    sum(bb_eom) as bb_eom,
    sum(tv_eom) as tv_eom,
    sum(vo_eom) as vo_eom
FROM join_to_dna
