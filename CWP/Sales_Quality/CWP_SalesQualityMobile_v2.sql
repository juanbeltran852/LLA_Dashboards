...
max(case when (month_survival = (select input_month from parameters) + interval '0' month  and (cast(serviceno as varchar) not in (SELECT cast(act_acct_cd as varchar) FROM "lla_cco_int_ext_dev"."drc_movil_new" WHERE date(concat(drc_period_final, '-01')) = (SELECT input_month FROM parameters)))) then 1 else null end )
